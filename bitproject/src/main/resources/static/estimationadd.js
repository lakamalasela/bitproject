        ...
        function loadTable(page,size,query) {
          ...
          var data = httpRequest("/reservation/findAll?page="+page+"&size="+size+query,"GET");
            if(data.content!= undefined) plansadd = data.content;
            createPagination('pagination',data.totalPages, data.number+1,paginate);
            fillTable('tblPlanadd',plansadd,fillForm,btnDeleteMC,viewitem);
            clearSelection(tblPlanadd);

            if(activerowno!="")selectRow(tblPlanadd,activerowno,active);
