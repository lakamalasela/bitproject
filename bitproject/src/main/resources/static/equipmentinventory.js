        ...
        function loadTable(page,size,query) {
            page = page - 1;
            equipmentsinventory = new Array();
          var data = httpRequest("/eqpinventory/findAll?page="+page+"&size="+size+query,"GET");
            if(data.content!= undefined) equipmentsinventory = data.content;
            createPagination('pagination',data.totalPages, data.number+1,paginate);
            fillTable('tblEqinventory',equipmentsinventory,fillForm,btnDeleteMC,viewitem);
            clearSelection(tblEqinventory);

            if(activerowno!="")selectRow(tblEqinventory,activerowno,active);
        ...
        function viewitem(cus,rowno) {
         ...
         function btnPrintRowMC(){
             ...
             newwindow.document.write("<html>" +
                 ...
                 "</address></div></div></di><hr> <h1>Equipment Inventory Details :</h1></div>" +
                 "<div>"+format+"</div>" +
                 "<script>printformtable.removeAttribute('style')</script>" +
                 "</body></html>");
             setTimeout(function () {newwindow.print(); newwindow.close();},100);
        ...
        function loadForm() {
            ...
            // fillCombo(cmbContact,"Select the contact media",contactmedias,"name","");
            //  // fillCombo(cmbDesignation,"Select Designation",designations,"name","");
            // fillCombo(cmbAssign,"Select Civil Status",employees,"callingname",session.getObject('activeuser').employeeId.callingname);
        ...
        function disableButtons(add, upd, del) {
            ...
            for(index in equipmentsinventory){
                if(equipmentsinventory[index].eistatus_id.name =="Deleted"){
                    tblEqinventory.children[1].children[index].style.color = "#f00";
                    tblEqinventory.children[1].children[index].style.border = "2px solid red";
                    tblEqinventory.children[1].children[index].lastChild.children[1].disabled = true;
                    tblEqinventory.children[1].children[index].lastChild.children[1].style.cursor = "not-allowed";
       ...
       function btnPrintTableMC(customer) {
            ...
            var newwindow=window.open();
            formattab = tblEqinventory.outerHTML;

           newwindow.document.write("" +
                "<html>" +
                "<head><style type='text/css'>.google-visualization-table-th {text-align: left;} .modifybutton{display: none} .isort{display: none}</style>" +
                "<link rel='stylesheet' href='../resources/bootstrap/css/bootstrap.min.css'/></head>" +
                "<body><div style='margin-top: 150px; '> <h1><img class='cnew' src='../resources/image/cnew.jpg'><span>Equipment Inventory Details :</span></h1></div>" +
                "<div>"+ formattab+"</div>"+
               "</body>" +
                "</html>");
           setTimeout(function () {newwindow.print(); newwindow.close();},100) ;
